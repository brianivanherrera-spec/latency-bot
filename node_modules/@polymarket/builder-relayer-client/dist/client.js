...
class RelayClient {
    ...
    async getDeployed(address, type) {
        ...
        const resp = await this.send(`${endpoints_1.GET_DEPLOYED}`, http_helpers_1.GET, { params });
    ...
    async deployDepositWallet() {
        ...
        const request = (0, builder_1.buildDepositWalletCreateRequest)(from, depositWalletConfig);
        const requestPayload = JSON.stringify(request);
        const resp = await this.sendAuthedRequest(http_helpers_1.POST, endpoints_1.SUBMIT_TRANSACTION, requestPayload);
        return new response_1.ClientRelayerTransactionResponse(resp.transactionID, resp.state, resp.transactionHash, this);
    }
    ...
    async executeDepositWalletBatch(calls, walletAddress, deadline) {
        ...
        const noncePayload = await this.getNonce(from, types_1.TransactionType.WALLET);
        const args = {
            from,
            chainId: this.chainId,
            walletAddress,
        ...
        const request = await (0, builder_1.buildDepositWalletBatchRequest)(this.signer, args, depositWalletConfig);
        const requestPayload = JSON.stringify(request);
        const resp = await this.sendAuthedRequest(http_helpers_1.POST, endpoints_1.SUBMIT_TRANSACTION, requestPayload);
        return new response_1.ClientRelayerTransactionResponse(resp.transactionID, resp.state, resp.transactionHash, this);
    }
     ...
     */
    async deriveDepositWalletAddress() {
        this.signerNeeded();
        const config = this.contractConfig.DepositWalletContracts;
        if (!(0, config_1.isDepositWalletContractConfigValid)(config)) {
            throw errors_1.CONFIG_UNSUPPORTED_ON_CHAIN;
        }
        const address = await this.signer.getAddress();
        return (0, builder_1.deriveDepositWallet)(address, config.DepositWalletFactory, config.DepositWalletImplementation);
    }
    /**
