...
class SignalEngine {
  ...
  _evaluate(currentPrice, currentTimestamp) {
    ...
    if (absZ < config.ZSCORE_THRESHOLD) {
      // Only log if very close to threshold (>95%) to reduce spam
      if (absZ >= config.ZSCORE_THRESHOLD * 0.95) {
        logger.info(`[SIGNAL_FILTER] reason=ZSCORE_TOO_LOW zscore=${zScore.toFixed(2)} threshold=${config.ZSCORE_THRESHOLD}`);
      }
      return null;
    }
    if (absMoveP < config.MOVE_PCT_THRESHOLD) {
      if (absMoveP >= config.MOVE_PCT_THRESHOLD * 0.95) {
        logger.info(`[SIGNAL_FILTER] reason=MOVE_PCT_TOO_LOW movePct=${movePct.toFixed(3)} threshold=${config.MOVE_PCT_THRESHOLD}`);
      }
      return null;
    }
    ...
    if (velocity < config.MIN_VELOCITY && !isExtremeMoveSignal) {
      if (velocity >= config.MIN_VELOCITY * 0.95) {
        logger.info(`[SIGNAL_FILTER] reason=VELOCITY_TOO_LOW velocity=${velocity.toFixed(6)} threshold=${config.MIN_VELOCITY}`);
      }
      return null;
    }
